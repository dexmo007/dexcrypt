...
public class Sha256 extends ShaAlgorithm {
    ...
    protected byte[] digestInternal() {
        final ByteBuffer buffer = ByteBuffer.allocate(Integer.BYTES * 8);
        buffer.putInt(h0);
        buffer.putInt(h1);
        buffer.putInt(h2);
        buffer.putInt(h3);
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        ...
        final byte[] message = "helloworld".getBytes(StandardCharsets.UTF_8);
        System.out.println(base64.encodeToString(MessageDigest.getInstance("SHA-256").digest(message)));
        System.out.println(base64.encodeToString(new Sha256().digest(message)));
    }
